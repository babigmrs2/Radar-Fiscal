...
0 7 * * * /usr/bin/python3 /caminho/completo/coletor.py >> /caminho/completo/coletor.- Windows Task Scheduler:
...
# ---------------------------------------------------------------------------
FONTES_FEDERAIS = [
{"nome": "Agência Senado", "url": "https://www12.senado.leg.br/noticias/rss/ultimas-noticias", {"nome": "Câmara dos Deputados", "url": "https://www.camara.leg.br/noticias/rss.xml", "esfera_{"nome": "Agência Brasil - Economia", "url": "https://agenciabrasil.ebc.com.br/rss/economia/{"nome": "Portal Contábeis", "url": "https://www.contabeis.com.br/rss/noticias/", "esfera_]
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
FONTES_ESTADUAIS = [
{"nome": "Sefaz-RS", "url": "https://www.sefaz.rs.gov.br/rss/noticias", "esfera_padrao": {"nome": "SEF/SC", "url": "https://www.sef.sc.gov.br/rss/noticias", "esfera_padrao": "estadual", {"nome": "SEFA-PR / Diário Oficial PR", "url": "https://www.aen.pr.gov.br/rss", "esfera_padrao": {"nome": "Sefaz-SP / Diário Oficial SP", "url": "https://www.doe.sp.gov.br/rss", "esfera_{"nome": "Sefaz-MG / Jornal Minas Gerais", "url": "https://www.jornalminasgerais.mg.gov.br/{"nome": "Economia-GO (Sefaz-GO)", "url": "https://www.economia.go.gov.br/rss/noticias", {"nome": "Sefaz-MS", "url": "https://www.sefaz.ms.gov.br/rss/noticias", "esfera_padrao": {"nome": "Sefaz-MT", "url": "https://www.sefaz.mt.gov.br/rss/noticias", "esfera_padrao": {"nome": "Sefaz-MA", "url": "https://www.sefaz.ma.gov.br/rss/noticias", "esfera_padrao": {"nome": "Sefaz-TO", "url": "https://www.sefaz.to.gov.br/rss/noticias", "esfera_padrao":
]
...
# ---------------------------------------------------------------------------
UF_MAPA = {
"RS": {"nome": "Rio Grande do Sul", "gentilicos": ["gaúcho", "gaúcha"], "orgaos": ["sefaz-"SC": {"nome": "Santa Catarina", "gentilicos": ["catarinense"], "orgaos": ["sef/sc", "sef "PR": {"nome": "Paraná", "gentilicos": ["paranaense"], "orgaos": ["sefa-pr", "sefa pr", ""SP": {"nome": "São Paulo", "gentilicos": ["paulista"], "orgaos": ["sefaz-sp", "sefaz sp", "MG": {"nome": "Minas Gerais", "gentilicos": ["mineiro", "mineira"], "orgaos": ["sefaz-mg", "GO": {"nome": "Goiás", "gentilicos": ["goiano", "goiana"], "orgaos": ["sefaz-go", "sefaz "MS": {"nome": "Mato Grosso do Sul", "gentilicos": ["sul-mato-grossense"], "orgaos": ["sefaz-"MT": {"nome": "Mato Grosso", "gentilicos": ["mato-grossense"], "orgaos": ["sefaz-mt", "sefaz "MA": {"nome": "Maranhão", "gentilicos": ["maranhense"], "orgaos": ["sefaz-ma", "sefaz ma"]},
"TO": {"nome": "Tocantins", "gentilicos": ["tocantinense"], "orgaos": ["sefaz-to", "sefaz }
...
PALAVRAS_FERTILIZANTES = ["fertilizante", "adubo", "npk", "ureia", "potássio", "fosfato", "amônia", PALAVRAS_MUNICIPAL = ["prefeitura", "município", "iss ", "issqn", "nfs-e"]
...
return "reforma"
if termo_bate_com_limite_de_palavra("ibs", texto_lower) or termo_bate_com_limite_de_palavra("return "reforma"
if any(p in texto_lower for p in PALAVRAS_MUNICIPAL):
return "municipal"
if uf is not None or any(p in texto_lower for p in PALAVRAS_ESTADUAL):
return "estadual"
return esfera_padrao if esfera_padrao in ("federal", "estadual", "municipal", "reforma") def classificar_setor(texto: str) -> str:
texto_lower = texto.lower()
...
try:
resposta = requests.get(url, timeout=TIMEOUT_SEGUNDOS, headers={"User-Agent": "RadarFiscalBot/resposta.raise_for_status()
...
return None
def dentro_da_janela_de_coleta(dt_publicacao: Optional[datetime], horas: int = JANELA_COLETA_"""Se a data não estiver disponível, mantemos a notícia (melhor incluir do que perder)."""
...
resumo_bruto = getattr(entrada, "summary", "") or getattr(entrada, "description", resumo = limpar_html(resumo_bruto)
...
def mesclar_com_historico(existentes: list, novas: list, dias: int = RETENCAO_DIAS, limite_itens: """Mescla notícias já salvas com as recém-coletadas, deduplica por link
...
combinadas = remover_duplicatas(novas + existentes) # novas primeiro: vencem em caso de combinadas = [item for item in combinadas if data_dentro_da_retencao(item, dias)]
...
log.info("Salvo %s com %d notícias (histórico de %d dias).", caminho, len(itens), RETENCAO_def main() -> None:
...
if not combinadas:
log.warning("Nenhuma notícia relevante (nova ou em histórico) — noticias.json não foi return
